...
import { QueryCommand, NativeAttributeValue, QueryCommandOutput } from "@aws-sdk/lib-dynamodb";
import { NextResponse, NextRequest } from "next/server";

import { dynamoDb } from "@/app/lib/dynamoClient";

type RawAppItem = Record<string, NativeAttributeValue> & {
  application_id?: string;
  title?: string;
  company?: string;
  interview_at?: string; // ISO UTC string
  url?: string;
  status?: string;
};

// Type guard: keep only items we can turn into InterviewItem
function isInterviewScheduled(it: RawAppItem): it is RawAppItem & {
  application_id: string;
  title: string;
  company: string;
  interview_at: string;
  ...
  return (
    it.status === "Interview Scheduled" &&
    typeof it.application_id === "string" &&
    typeof it.title === "string" &&
    typeof it.company === "string" &&
    typeof it.interview_at === "string"
  );
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    let allItems: RawAppItem[] = [];
    let lek: Record<string, NativeAttributeValue> | undefined;
    do {
      const resp : QueryCommandOutput = await dynamoDb.send(
